...
const localDbPath = "C:\\Users\\minec\\OneDrive\\Desktop\\вкф\\qq\\botdata.db";
const originalDbPath = fs.existsSync(localDbPath) ? localDbPath : path.resolve(__dirname, botdata.db);
const tmpDbPath = path.resolve(__dirname, botdata.db);

// Копируем базу данных в /tmp
if (!fs.existsSync(tmpDbPath)) {
  if (fs.existsSync(originalDbPath)) {
    fs.copyFileSync(originalDbPath, tmpDbPath);
    console.log('База данных успешно скопирована в /tmp');
  } else {
    console.error('Ошибка: Оригинальный файл базы данных не найден!');
    process.exit(1); // Завершаем процесс, если база данных отсутствует
  }
}

// Подключение к базе данных из /tmp
...
db.serialize(() => {
  db.get('SELECT name FROM sqlite_master WHERE type="table" LIMIT 1', [], (err, row) => {
    if (err) {
      ...
      console.log('Таблицы в базе данных отсутствуют.');
    } else {
      console.log('Пример подключения к базе данных успешен. Таблица найдена:', row);
    }
  });
});
// Дальнейшая логика сервера
function sendTelegramNotification(message) {
    sendNotification(message);
}

// Все остальное содержимое остается без изменений
